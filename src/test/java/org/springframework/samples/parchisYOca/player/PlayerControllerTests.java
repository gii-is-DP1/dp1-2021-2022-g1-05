...
public class PlayerControllerTests {
    ...
    void setup() {
        ...
        given(this.playerService.findPlayerById(TEST_PLAYER_ID).get()).willReturn(ManuK);*/
    ...
    void testProcessCreationFormSuccess() throws Exception {
        mockMvc.perform(post("/users/new").param("email", "manutest@gmail.com")
                .param("user.password", "1234567")
    ...
    void testProcessCreationWithWrongEmail() throws Exception {
        mockMvc.perform(post("/users/new").param("email", "")
            .param("user.password", "1234567")
            .param("user.username", "ManuK2").with(csrf()))
            .andExpect(view().name("users/createPlayerForm"));
    }
    ...
    void testProcessCreationWithRepeatedUsername() throws Exception {
        mockMvc.perform(post("/users/new").param("email", "manutest@gmail.com")
            .param("user.password", "1234567")
            .param("user.username", "ManuK").with(csrf())).andExpect(status().is3xxRedirection());
    }

    @WithMockUser(value = "spring")
    @Test
    void testProcessCreationWithRepeatedEmail() throws Exception {
        mockMvc.perform(post("/users/new").param("email", "manu@gmail.com")
            .param("user.password", "1234567")
            .param("user.username", "ManuK2").with(csrf())).andExpect(status().is3xxRedirection());
    }
